...
DROP POLICY IF EXISTS "Anyone can view content library" ON content_library;
CREATE POLICY "Anyone can view content library"
  ON content_library FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "Anyone can insert to content library" ON content_library;
CREATE POLICY "Anyone can insert to content library"
  ON content_library FOR INSERT
  TO authenticated
  WITH CHECK (true);

DROP POLICY IF EXISTS "Anyone can update content library" ON content_library;
