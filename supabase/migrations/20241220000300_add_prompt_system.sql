...
DROP POLICY IF EXISTS "Anyone can view active global prompts" ON prompt_templates;
...
DROP POLICY IF EXISTS "Admins can manage global prompts" ON prompt_templates;
...
DROP POLICY IF EXISTS "Anyone can view versions of visible prompts" ON prompt_versions;
...
DROP POLICY IF EXISTS "Admins can manage prompt versions" ON prompt_versions;
...
DROP POLICY IF EXISTS "Users can manage own preferences" ON learning_preferences;
...
DROP POLICY IF EXISTS "Users can view own customizations" ON user_prompt_customizations;
CREATE POLICY "Users can view own customizations"
  ON user_prompt_customizations FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can manage own customizations" ON user_prompt_customizations;
...
DROP POLICY IF EXISTS "Anyone can view active locations" ON location_metadata;
CREATE POLICY "Anyone can view active locations"
  ON location_metadata FOR SELECT
  TO authenticated
  USING (is_active = TRUE);

DROP POLICY IF EXISTS "Only admins can manage locations" ON location_metadata;
...
DROP TRIGGER IF EXISTS update_learning_preferences_updated_at ON learning_preferences;
CREATE TRIGGER update_learning_preferences_updated_at
  BEFORE UPDATE ON learning_preferences
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_user_prompt_customizations_updated_at ON user_prompt_customizations;
...
DROP TRIGGER IF EXISTS auto_version_prompt_templates ON prompt_templates;
